...
class KDLQRApp:
    ...
    def print_qr(self):
        ...
        try:
            ...
            # Fix für Umlaute in Drucker-/Treibernamen
            if isinstance(dev_name, bytes):
                dev_name = dev_name.decode('mbcs', errors='replace')
            if isinstance(dev_driver, bytes):
                dev_driver = dev_driver.decode('mbcs', errors='replace')
            if isinstance(dev_output, bytes):
                dev_output = dev_output.decode('mbcs', errors='replace')
            ...
            import traceback
            messagebox.showerror("Druckfehler", f"{e}\n\n{traceback.format_exc()}")
